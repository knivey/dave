...
func createServer(cfg Config, handlers *ToolHandlers) *mcp.Server {
	server := mcp.NewServer(
		...
		&mcp.ServerOptions{
			Instructions: "MCP server for ComfyUI image generation and prompt enhancement. Use generate_image or enhance_and_generate for one-step image generation (blocks until done). Use generate_image_async or enhance_and_generate_async to queue jobs and get a job_id immediately, then check with job_status or wait_for_job.",
		},
	)
